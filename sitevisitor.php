 ...
 * Plugin Name: Site Visitor Info  
   Description: Block Visitor User and Redirect them to specific URL.
 * Author:Usman Altaf
 * Version:1.0
 * License:GPL2
 *
...
{ 
	...
	public function __construct(){
		...
		add_action( 'admin_menu', array( $this, 'svi_visitor_plugin_page' ) );
		$this->svi_hooks();
	} 

	public function svi_visitor_plugin_page() {

		add_menu_page(__('Site Visitor Info'), __("Site Visitor Info"), 'administrator', "site_visitor",  array( $this, 'svi_site_visitor_html_page' ));
	}

	public function svi_hooks(){
		
		// activating  database table
		register_activation_hook( __FILE__, array($this, 'svi_site_visitor_info_database_table') );
		
		add_action( 'init', array($this, 'init') );

	}

	public function svi_site_visitor_info_database_table() {
	...
	public function svi_site_visitor_html_page() {
		...
		$return_output .='
		    <div class="wrap">
			<form class="site_visited" style="margin: 27px 0px;" method="post" action=""><input class="redirect" name="redirect" type="text" placeholder="Enter the Url" value="'.$redirect_val.'"><input class="button button-primary unblock" name="submit" type="submit" value="send"></form>
	...
	public function init(){
		...
		$User_info = $this->svi_Check_User_info();
		...
		if($User_info){
			...
			if($User_action == 'banned'){
				$this->svi_site_redirect();
			}
		}

		else{ 

			$browser	= $this->svi_browser_data();
			global $wpdb; 
		$table_name = $wpdb->prefix . 'visitor_details'; 
	...
	public function svi_Check_User_info(){
		global $wpdb; 
		$table_name = $wpdb->prefix . 'visitor_details'; 
		$userip = $_SERVER['REMOTE_ADDR'];
	...
	private function svi_browser_data() {
	...
	public function svi_site_redirect(){
		$redirect_val = get_option( 'redirect' );
